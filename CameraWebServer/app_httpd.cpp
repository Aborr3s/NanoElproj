...
// GPIO pin driven HIGH when a face is detected, stays HIGH for FACE_HOLD_US
// microseconds
#define FACE_DETECT_PIN 12
#define FACE_HOLD_US 3000000LL            // 3 seconds
static int64_t face_detect_timestamp = 0; // time of last face detection (us)
...
static int8_t detection_enabled = 1; // Enabled by default at boot
  ...
  while (true) {
...
#endif
    face_id = 0;
    // Drive pin LOW only after hold time has expired
    if ((esp_timer_get_time() - face_detect_timestamp) > FACE_HOLD_US) {
...
#endif
            draw_face_boxes(&rfb, &results, face_id);
            digitalWrite(FACE_DETECT_PIN, HIGH);          // Face detected!
            face_detect_timestamp = esp_timer_get_time(); // Restart hold timer
          }
    ...
    ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
              ...
              if (results.size() > 0) {
                ...
                draw_face_boxes(&rfb, &results, face_id);
                digitalWrite(FACE_DETECT_PIN, HIGH); // Face detected!
                face_detect_timestamp =
                    esp_timer_get_time(); // Restart hold timer
              }
...
// ---------------------------------------------------------------------------
// Standalone face detection task
// Runs in the background independently of the HTTP stream.
// GPIO 12 stays HIGH for FACE_HOLD_US after each detection.
// ---------------------------------------------------------------------------
#if CONFIG_ESP_FACE_DETECT_ENABLED
static void face_detect_task(void *arg) {
  ...
  while (true) {
    ...
    if (fb && fb->width <= 400) {
      ...
      if (fb->format == PIXFORMAT_RGB565) {
        // Fast path: detect directly on RGB565 frame
#if TWO_STAGE
        ...
        std::list<dl::detect::result_t> &res =
            s1.infer((uint16_t *)fb->buf, {(int)fb->height, (int)fb->width, 3});
#endif
        face_found = (res.size() > 0);
      } else {
        // Slow path: decode JPEG/other format to RGB888 first
...
#if TWO_STAGE
            std::list<dl::detect::result_t> &cands =
                s1.infer((uint8_t *)out_buf, {(int)fb->height, (int)fb->width, 3});
            std::list<dl::detect::result_t> &res =
                s2.infer((uint8_t *)out_buf, {(int)fb->height, (int)fb->width, 3}, cands);
#else
            std::list<dl::detect::result_t> &res =
                s1.infer((uint8_t *)out_buf, {(int)fb->height, (int)fb->width, 3});
#endif
      ...
      if (face_found) {
        ...
        face_detect_timestamp = esp_timer_get_time();
      } else if ((esp_timer_get_time() - face_detect_timestamp) > FACE_HOLD_US) {
        digitalWrite(FACE_DETECT_PIN, LOW);
      }
    }
    ...
    vTaskDelay(30 / portTICK_PERIOD_MS); // ~30fps detection rate
  }
}
#endif // CONFIG_ESP_FACE_DETECT_ENABLED

void startFaceDetectionTask() {
#if CONFIG_ESP_FACE_DETECT_ENABLED
  xTaskCreate(face_detect_task, "face_detect", 8192, NULL, 5, NULL);
  log_i("Face detection background task started on GPIO %d", FACE_DETECT_PIN);
#endif
}
